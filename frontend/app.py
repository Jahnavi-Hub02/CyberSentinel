...
def normalize_incident_data(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure key incident columns are present and free of NaNs."""
...
def main() -> None:
    ...
    # normalize dataframe to canonical columns and fill key fields
    if not df.empty:
        df = normalize_df(df)
        df = normalize_incident_data(df)
