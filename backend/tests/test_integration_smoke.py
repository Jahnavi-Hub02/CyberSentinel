...
def test_incidents_limit_and_enrichment_behavior():
    ...
    assert any(
        isinstance(x.get("latitude"), (int, float)) and isinstance(x.get("longitude"), (int, float))
        for x in all_data
    ), "expected at least one incident with latitude/longitude in full response"
    ...
    if any(e.get("location") for e in expected):
        assert any(
            x.get("latitude") is not None and x.get("longitude") is not None for x in lim_data
        ), "limited response missing latitude/longitude for known locations"
